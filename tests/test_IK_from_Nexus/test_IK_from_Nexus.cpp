...
int main() {
    ...
    std::string osimModelFileName = dataDir + "gait2392_full.osim";
    std::string ikTaskFileName(dataDir + "gait2392-tasks.xml");

    std::string hostname("127.0.0.1:801");
    unsigned nThreads(4); // amount of threads to use in multithreading
    ...
    //rawIkLogger.setConvertFromRadToDeg();

    std::cout << "Reading frames from generalisedCoordinatesQueue\n";
    ...
    std::cout << "Setting count of subscriptions and executions\n";
